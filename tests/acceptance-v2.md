...
All safety/architecture scenarios must pass before stable v2 promotion. Runtime-dependent scenarios are executed in a real OpenClaw installation. Static CI must validate package/contracts/gate behavior; existence of this document is never proof that runtime scenarios passed.

## Architecture and learning

### A1 Single agent learning
A permanent agent completes/fails a task. Verification selects factual Evidence and produces a VerificationResult; Experience derives a scoped lesson from supporting Evidence only after the relevant outcome is evaluated; no parallel task/memory runtime is created.
...
Simulate OpenClaw exposing a FULL-equivalent native capability. Delegate to that exposed native capability without changing four-Core semantics; a real fallback, if one exists, becomes removable. A declarative registry entry alone is not runtime proof.
...
### A8 Unknown safety
Insufficient evidence yields UNKNOWN. UNKNOWN cannot autonomously create durable causal/general/shared rules or authorize risky evolution. A narrow observability lesson is allowed only when the missing visibility itself is directly evidenced.
...
OpenClaw capability change updates provider selection/native-first audit rather than version-coupled Core logic. Declarative registry expectations must not be treated as detected runtime capabilities.
...
For an exposed substantive task containing a durable reusable correction: automatically select Evidence -> produce VerificationResult -> derive scoped Experience -> use native persistence if available -> allow later native recall -> re-check against future outcome, with no Agent OS-specific user setup.

## Semantic contract closure

### A31 Evidence is not verdict
Given a successful tool assertion without independent outcome evidence, it may become Evidence but cannot itself be serialized/treated as a PASS VerificationResult. VerificationResult criteria must point to supporting Evidence IDs.

### A32 Experience admission is verification-bound
Given Evidence with relevant VerificationResult UNKNOWN, do not derive a durable causal/general Experience. Given directly evidenced missing observability, a narrow observability/process Experience may be admitted without pretending the task outcome is known.

### A33 Governance state machine
A candidate progresses CANDIDATE -> REVIEW/APPROVED -> APPLIED -> VERIFIED only with the required governance/native observations. APPROVED must not imply APPLIED; APPLIED must not imply VERIFIED. REJECT maps to REJECTED, and invisible/unavailable native application cannot advance status.

### A34 Contract/schema alignment
Representative valid AgentIdentity, DelegationTrace, Evidence, VerificationResult, Experience, EvolutionCandidate and Capability objects must validate against their schemas; known drift cases (wrong Evidence provenance type, arbitrary source, stale `experiences` field, invalid nullable fields, extra properties) must fail.
