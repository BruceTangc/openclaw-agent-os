...
objs={p:load_json(p) for p in json_files if (ROOT/p).exists()}
skill=read('SKILL.md')
...
ids=[int(x) for x in re.findall(r'^###\s+A(\d+)\b',accept,re.M)]
expected=list(range(1,ACCEPTANCE_MAX+1))
if ids != expected: errors.append(f'acceptance headings must be exactly A1-A{ACCEPTANCE_MAX} in order; got {ids}')
...
# Frozen schema contract checks. These are intentionally explicit so schema drift fails CI.
def schema(path): return objs.get(path,{})
def assert_object_schema(path, required_fields):
    s=schema(path)
    ...
    if set(s.get('required',[]))!=set(required_fields): errors.append(f'{path}: required fields drift')
    if set(s.get('properties',{}))!=set(required_fields): errors.append(f'{path}: properties drift')

assert_object_schema('schemas/agent-identity.schema.json',['agent_id','kind','root_agent_id','requester_agent_id','parent_agent_id','session_id','parent_session_id','task_id','parent_task_id','delegation_path'])
assert_object_schema('schemas/evidence.schema.json',['id','timestamp','source','identity','goal','success_criteria','action','observation','feedback','scope','confidence','provenance'])
assert_object_schema('schemas/verification-result.schema.json',['id','target','status','criteria_results','reason','confidence'])
assert_object_schema('schemas/experience.schema.json',['id','derived_from','type','situation','action','outcome','lesson','confidence','occurrences','scope','owner_agent_id','team_id','contradicts'])
assert_object_schema('schemas/evolution-candidate.schema.json',['id','experience_ids','pattern','hypothesis','target','expected_improvement','proposed_change','confidence','risk','status'])
assert_object_schema('schemas/delegation-trace.schema.json',['requester_agent_id','executor_agent_id','root_agent_id','parent_task_id','child_task_id','requester_session_id','executor_session_id','delegated_goal','success_criteria','artifact_refs'])
assert_object_schema('schemas/capability.schema.json',['id','provider','support','version_hint','contract_version','notes'])
...
enum('schemas/agent-identity.schema.json','kind',['ROOT','PERMANENT','SPECIALIST','SUBAGENT','ACP','UNKNOWN'])
enum('schemas/evidence.schema.json','source',['TOOL','ENVIRONMENT','USER','AGENT','TEST','REVIEW','NATIVE_EVENT'])
enum('schemas/evidence.schema.json','scope',['RUN','SESSION','TASK','DELEGATION'])
enum('schemas/verification-result.schema.json','target',['RUN','TASK','DELEGATION','USER_OUTCOME'])
enum('schemas/verification-result.schema.json','status',['PASS','PARTIAL','FAIL','UNKNOWN'])
enum('schemas/experience.schema.json','type',['SUCCESS','FAILURE','WORKFLOW','TOOL','DELEGATION','USER_INTERACTION'])
enum('schemas/experience.schema.json','scope',['AGENT','TEAM','SHARED'])
enum('schemas/evolution-candidate.schema.json','target',['AGENT','TEAM','SKILL','WORKFLOW','SHARED_PROTOCOL'])
enum('schemas/evolution-candidate.schema.json','risk',['LOW','MEDIUM','HIGH','CRITICAL'])
enum('schemas/evolution-candidate.schema.json','status',['CANDIDATE','REVIEW','APPROVED','REJECTED','APPLIED','VERIFIED'])
enum('schemas/capability.schema.json','provider',['OPENCLAW_NATIVE','OPENCLAW_OFFICIAL_PLUGIN','AGENT_OS_ADAPTER','AGENT_OS_FALLBACK'])
...
def scan_python(path,text):
    ...
    for node in ast.walk(tree):
        if isinstance(node,(ast.Assign,ast.AnnAssign)):
            value=node.value; v=const_text(value,env) if value is not None else None
    ...
    for node in ast.walk(tree):
        ...
        if v is not None and is_retired(v):
            ...
            if key not in seen: seen.add(key); errors.append(f'executable legacy reference: {path.relative_to(ROOT)}:{key[0]} -> {key[1]}')
...
print(f'checked {len(required)} canonical artifacts, A1-A{ACCEPTANCE_MAX}, frozen schema contracts, version coherence, declarative native boundary, AST-aware legacy references, and absence of v1.3 package artifacts')
