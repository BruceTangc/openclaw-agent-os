...
MUTATION_SELF=(ROOT/'scripts/test_validate_v2.py').resolve()
ACCEPTANCE_MAX=34
...
scan_roots=[ROOT/'scripts',ROOT/'tests',ROOT/'.github'/'workflows'];text_ext={'.py','.sh','.ps1','.js','.mjs','.cjs','.ts','.tsx','.yml','.yaml'}
for base in scan_roots:
    if not base.exists():continue
    for path in base.rglob('*'):
        # The validator and its mutation suite necessarily contain the denylist/test payloads.
        if not path.is_file() or path.suffix.lower() not in text_ext or path.resolve() in {SELF,MUTATION_SELF}:continue
...
print(f'checked {len(required)} canonical artifacts, A1-A{ACCEPTANCE_MAX}, frozen schema contracts, version/manifest coherence, declarative native boundary, AST-aware legacy references, and absence of v1.3 package artifacts')
