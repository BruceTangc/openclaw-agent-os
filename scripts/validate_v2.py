#!/usr/bin/env python3
"""Static package/architecture gate for Agent OS v2. No third-party dependencies."""
...
for term in ['Verification','Experience','Evolution','Governance','OpenClaw','multi-agent','Zero-configuration']:
...
if '2.0.0-rc.1' not in skill: errors.append('root SKILL version mismatch')
for p in ['SKILL.md','protocols/MULTI-AGENT.md','docs/ARCHITECTURE-V2.md']:
    ...
    if pos >= 0 and 'Never' not in t[max(0,pos-120):pos]: errors.append(f'hard-coded main-agent ownership in {p}')
if errors:
    print('Agent OS v2 gate: FAIL')
    for e in errors: print('-',e)
    sys.exit(1)
print('Agent OS v2 gate: PASS')
print(f'checked {len(required)} required artifacts, zero-config packaging, and stable JSON contracts')
