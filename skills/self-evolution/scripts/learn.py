    ...
    try:
        ...
        if lock_fd:
            try:
                ...
                pass
            lock_fd.close()
GLOBAL_LOCK_PATH = os.path.join(WORKSPACE, "memory", "agents", ".learning-os.lock")


def acquire_global_lock():
    """统一全局锁（规范 §31）：Learning OS + Ontology 的 mutation 属于同一 Global Cycle。
    拿不到锁返回 None（有另一轮在跑），调用方应跳过本轮。"""
    try:
        import fcntl
        os.makedirs(os.path.dirname(GLOBAL_LOCK_PATH), exist_ok=True)
        fd = open(GLOBAL_LOCK_PATH, "w")
...
def run_full_cycle(trail, auto_promote=True, auto_summary=True):
    # 统一全局锁（规范 §31）：防止 cron 与手动 --cycle 并发跑
    _cycle_lock_fd = acquire_global_lock()
    if _cycle_lock_fd is None:
        print("⏭  Global learning cycle skipped: another cycle holds .learning-os.lock")
        return
    now = datetime.now()
    ...
    # Phase 0.5: Ontology × Self-Improvement — 语义富化（集成规范 V1.0 §5/§33）
    # Ontology 解析插在 Bus Drain 之后、Learning Engine 决策之前：
    # 只提供语义上下文（实体/关系/影响），不决定 Learning 是否成立或晋升。
    print(f"🧠 Phase 0.5: Ontology enrichment")
    try:
        import subprocess
        bridge = os.path.join(os.path.dirname(__file__), "ontology_bridge.py")
        if os.path.exists(bridge):
            result = subprocess.run(
                [sys.executable, bridge, "--enrich", "--limit", "20"],
                capture_output=True, text=True, timeout=60
            )
            for line in result.stdout.split('\n'):
                s = line.strip()
                if s.startswith("✓") or s.startswith("○") or s.startswith("✗") or s.startswith("   "):
                    if "enriched" not in s and "候选" not in s and "Bridge" not in s:
                        print(f"   {s}")
            # bridge 是子进程，富化结果已写盘；但 learn.py 内存里的 trail 是旧数据，
            # 后续 Phase 9/10 的 save_trail 会覆盖丢失富化。重新读取磁盘并合并回内存。
            try:
                with open(LEARNING_TRAIL_PATH, encoding="utf-8") as _f:
                    disk_trail = json.load(_f)
                disk_map = {e.get("id"): e for e in disk_trail.get("entries", [])}
                merged = 0
                for ent in trail.get("entries", []):
                    d = disk_map.get(ent.get("id"))
                    if d and (d.get("extra_meta") or {}).get("ontology"):
                        ent.setdefault("extra_meta", {})["ontology"] = d["extra_meta"]["ontology"]
                        merged += 1
                if merged > 0:
                    print(f"   ↪ Merged ontology enrichment back into {merged} trail entr(ies)")
            except Exception as _e:
                print(f"   Merge back failed (non-fatal): {_e}")
            # 用正则解析 bridge 输出里实际的 enriched 数量（"完成: N enriched"）
            import re as _re
            _m = _re.search(r"完成:\s*(\d+)\s+enriched", result.stdout)
            _enriched_n = int(_m.group(1)) if _m else 0
            if _enriched_n > 0:
                print(f"   ✅ Ontology context enriched for {_enriched_n} learning entr(ies)")
                actions_taken.append(f"🧠 Enriched {_enriched_n} entries with Ontology semantic context")
            elif _enriched_n == 0:
                print(f"   (no new entries to enrich)")
            if result.returncode != 0 and result.stderr.strip():
                print(f"   Bridge stderr: {result.stderr.strip()[:200]}")
        else:
            print(f"   ontology_bridge.py not found, skipping (ontology skill not installed)")
    except Exception as e:
        print(f"   Ontology enrichment failed: {e}")
    ...
    release_global_lock(_cycle_lock_fd)
