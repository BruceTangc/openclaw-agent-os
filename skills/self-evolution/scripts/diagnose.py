...
# -*- coding: utf-8 -*-
"""
diagnose.py — Self-Evolution v2 · Diagnose (Candidate → Diagnosis)

职责：
  candidate 必须先 diagnose：
    valid?  root cause?  reproducible?  external factor?  existing solution?
    target?  change level?  confidence?
...
只有全部满足才允许进 Proposal：
    valid=true AND reproducible=true AND external_environment=false AND
    existing_solution=false AND confidence 达标

状态：CANDIDATE → DIAGNOSED（通过） / → UNRESOLVED（不通过/证据不足）
Code = Enforcement：本脚本判定通过/打回，不依赖 LLM 编造。

用法：
  python3 diagnose.py --candidate CAND-xxx --root_cause workflow_gap \
      --valid --reproducible --confidence 0.8 --level G3 \
      --target ~/.openclaw/workspace/skills/xxx/SKILL.md
"""
...
def evaluate(cand_id, root_cause, valid, reproducible, external, existing,
    ...
    if external or root_cause in ("external_environment", "user_requirement"):
        ...
        dgn = {
            ...
            "target": target,
            "level": level,
            "reason": "external_environment_or_user_requirement",
        }
        did = _core.save_artifact("diagnosis", dgn)
        _core.assert_transition(dgn, "UNRESOLVED", kind="diagnosis")
        _core.save_artifact("diagnosis", dgn)
        return did, None

    gates = {
        "valid": valid,
        "reproducible": reproducible,
        "external_environment_not": (not external),
        "no_existing_solution": (not existing),
        "confidence_ok": float(confidence) >= CONFIDENCE_MIN,
    }
    if not all(gates.values()):
        # 未达标 → UNRESOLVED，保留记录不静默丢弃
        _core.assert_transition(cand, "UNRESOLVED", kind="candidate")
        _core.save_artifact("candidate", cand)
        dgn = {
            "status": "UNRESOLVED",
            ...
            "external_factor": bool(external),
            "existing_solution": bool(existing),
            "confidence": float(confidence),
            "target": target,
            "level": level,
            "reason": "diagnosis_gate_failed",
            "gates": gates,
        }
        did = _core.save_artifact("diagnosis", dgn)
        return did, None

    # 保护目标拦截（diagnose 阶段即拦截，防 Proposal 触达 Permission/Security）
    if _core.is_protected_target(target):
        raise ValueError("目标 {} 受保护，不能进入演进（Permission/Security/Runtime 永不自动改）".format(target))
...
def main():
    p = argparse.ArgumentParser(description="Self-Evolution v2 Diagnose")
    p.add_argument("--candidate", required=True)
    p.add_argument("--root_cause", required=True,
                   choices=ALLOWED_ROOT_CAUSES)
    ...
    if err:
        print(json.dumps({"decision": "REJECT", "reason": err},
                         ensure_ascii=False, indent=2))
        return
    dgn = _core.load_artifact("diagnosis", did)
    print(json.dumps({"decision": "DIAGNOSED" if dgn.get("status") == "DIAGNOSED"
                      else "UNRESOLVED",
                      "diagnosis_id": did,
                      "status": dgn.get("status"),
                      "root_cause": dgn.get("root_cause"),
                      "level": dgn.get("level")},
                     ensure_ascii=False, indent=2))
