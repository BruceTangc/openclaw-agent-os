...
def approve_draft(name):
    ...
    # Skill Evolution 回写 ontology（规范 §21/§26）：新 skill 注册进语义层
    try:
        import subprocess
        import sys
        ...
        if os.path.exists(bridge):
            proc = subprocess.run(
                [sys.executable, bridge, "--sync-skill", "--skill_name", name],
                capture_output=True, text=True, timeout=30)
            out = proc.stdout.strip()
            if out:
                print(out)
        else:
            print("  (ontology skill 未安装，跳过语义回写)")
    except Exception as e:
        print(f"  (ontology 回写失败: {e})")
