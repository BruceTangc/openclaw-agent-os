...
> **CORE-19（最小核心类型白名单）**：上述 20 个类型 + `Person` 构成最小核心白名单
> （脚本内 `CORE_ENTITY_TYPES`）。`--type` 会先由 schema 校验，再对照白名单：
> 不在白名单的类型可存在（自定义扩展），但会收到 **CORE-19 告警**，提示走语义模型评审,
> 防止实体类型自由膨胀。想持久化新核心类型 → 提交语义模型变更评审后再纳入。
